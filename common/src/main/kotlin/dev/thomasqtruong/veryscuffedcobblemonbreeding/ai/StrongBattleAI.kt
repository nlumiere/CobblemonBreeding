...
fun getDamageMultiplier(attackerType: ElementalType, defenderType: ElementalType): Double {
    return com.cobblemon.mod.common.battles.ai.typeEffectiveness[attackerType]?.get(defenderType) ?: 1.0
...
class StrongBattleAI(skill: Int) : BattleAI {
    ...
    fun highestDamagePossible(selectedMove: InBattleMove, moveset: ShowdownMoveset?, mon: ActiveTracker.TrackerPokemon, opponent: ActiveTracker.TrackerPokemon, currentWeather: String?): Double {
        var damage = 0.0
        if (moveset != null) {
            for (move in moveset.moves.filter { !it.disabled && it.id != selectedMove.id}) {
                val newDamage = calculateDamage(move, mon, opponent, currentWeather)
                if (newDamage > damage) {
                    damage = newDamage
                }
            }
    ...
    override fun choose(activeBattlePokemon: ActiveBattlePokemon, moveset: ShowdownMoveset?, forceSwitch: Boolean): ShowdownActionResponse {
        ...
        if (shouldSwitchOut(request, battle, activeBattlePokemon, moveset)) {
            ...
            for (move in moveset!!.moves.filter { !it.disabled }) {
                if (move.pp > 0 && move.id in pivotMoves && moveDamageMultiplier(move.id, opponent) != 0.0) {
                    return chooseMove(move, activeBattlePokemon)
                }
            }
        ...
        if (!moveset?.moves?.isEmpty()!! && !shouldSwitchOut(request, battle, activeBattlePokemon, moveset) ||
            ...
            // Boosting moves like swords dance etc
            // todo if move is a boosting move and that current stat is less than 2 boosts then do it
            // todo fix it where positive and negative boosts are not being tracked by the Context Manager anymore :(
            /*for (move in moveset.moves.filter { !it.disabled }) {
                if (move.pp > 0) {
                    val minStatBoost = getNonZeroStats(move.id).keys.minOfOrNull { stat ->
                        mon.boosts[stat] ?: 0
                    } ?: 99 // Large number to break
...
// maybe use pokemon.species to compare to active pokemon
    ...
    private fun updateActiveTracker(activeBattlePokemon: ActiveBattlePokemon, request: ShowdownActionRequest, battle: PokemonBattle) {
        ...
        p1.activePokemon.currentTypes = pokemon1.types.map { it.toString() }.toMutableList()
        ...
        p2.activePokemon.currentTypes = pokemon2.types.map { it.toString() }.toMutableList()
        //mon.stats = pokemon.stats
